...
tab('支撑材料文件列表',['文件或目录','用途'],[['src/model.py','数据解析、预测、调度及物理与费用检查'],['src/run.py','四问计算、参数选型及敏感性实验'],['src/q1_certificate.py','独立线性规划及最优性检验'],['src/review_results.py','结果表独立回读与指定日期汇总'],['src/deliver.py中的causality函数','未来输入污染测试'],['src/figures.py','计算结果图形生成'],['results/*.npz；*_daily.csv','逐时段轨迹及逐日账单'],['audit/*.csv；*.json','数据与约束检查'],['06_错误说明与纠正/问题1至问题4','五份修订结果工作簿']],[7,9])
...
for k,fname in enumerate(['model.py','run.py','review_results.py','q1_certificate.py','deliver.py','figures.py']):
    br();h(f'附录{chr(66+k)}　{fname}源程序')
    source=(SRC/fname).read_text(encoding='utf-8')
    if fname=='deliver.py':
...
OUT=HERE/'微网购电策略论文_范例风格完善版.docx';doc.save(OUT)
